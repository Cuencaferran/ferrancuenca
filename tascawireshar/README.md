...
Anàlisi de tràfic amb Wireshark (INCIBE=Institut de Ciberseguretat d’Espanya): https://www.incibe.es/extfrontinteco/img/File/intecocert/EstudiosInformes/cert_inf_seguridad_analisis_trafico_wireshark.pdf
...
Introducció (teoria)
L'eina que farem servir és Wireshark, que ja tenim instal·lat al Kali Linux. El protocol 'ICMP`(Internet Control Message Protocol) permet als dispositius de xarxa informar d'una errada. Per exemple, si un router no pot encaminar un paquet, el descarta i torna un error ICMP a l'emissor, o si un paquet acaba el seu TTL (Time To Live), el router el descarta i envia un missatge ICMP a l'emissor.

Ping és un dels serveis que ofereix ICMP. Serveix per comprovar si es pot accedir a un equip a la xarxa. També ens diu el temps que es triga en obtenir resposta. Traceroute ens diu per quins routers passen els paquets fins arribar al host destí.
